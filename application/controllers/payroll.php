...
class Payroll extends CI_Controller
{
	...
	//function index		
	function index1($offset = 0, $order_column = 'last_name', $order_type = 'asc')
	{
		...
		// load data
		$Users = $this->payrollmodel->get_paged_list($limit, $offset, $order_column, $order_type, $filter)->result();
		// generate pagination
		$this->load->library('pagination');
		$config['base_url'] = site_url('/payrollruns/index');
		$config['total_rows'] = $this->payrollmodel->count_all();
		...
		//anchor('payrollruns/index/'.$offset.'/Min Range/'.$new_order, 'Min Range'),
       		'Year','Month','Payroll Coverage', 'Date Posted','Actions');
		...
		foreach ($Users as $Users) {
           ...
           // $Users->status,
			...
			anchor('payroll/view/'.$Users->payr_id,'View',array('class'=>'view'))
		...
		$this->load->view('pages/template/header_orig');
	...
	function index($offset = 0, $order_column = 'last_name', $order_type = 'asc')
	{
        ...
        $emp_no = $this->payrollmodel->get_employee_no($un);
	   
		$lname = $this->payrollmodel->get_employee_details($emp_no)->row()->last_name;
		$fname = $this->payrollmodel->get_employee_details($emp_no)->row()->first_name;

		//$net_pay = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->net_pay;


		$limit =  100;
		$Users = $this->payrollmodel->get_processed_payroll($limit, $offset, $order_column, $order_type, $emp_no)->result();
		
		$this->load->library('pagination');
		$config['base_url'] = site_url('/payroll/view');
		$config['total_rows'] = $this->payrollmodel->count_all();
		...
		//$pr = $this->payrollmodel->get_payrollrun($id)->result();
		

		//foreach ($pr as $pr){
		$data['title'] = 'Processed Payroll of '.$lname.', '.$fname;
	   ...
	   // }
		...
		$this->table->set_empty("");
		$new_order = ($order_type == 'asc' ? 'desc' : 'asc');
		$this->table->set_heading('Coverage','Basic Salary', 'Total De minimis ','Salary Adjustment','Withholding Tax', 'SSS Contribution', 'Philhealth Contribution','HDMF Contribution','SSS Loan','Pagibig Loan','Other Deductions','Total Deduction','Net Pay','Payslip');
		...
		foreach ($Users as $Users) {
			...
			//anchor_popup('payrollruns/payr_adjustments/'.$Users->payr_detail_id,'Update',array('class'=>'update'), $upd).'&nbsp &nbsp &nbsp'.
			//anchor_popup('payrollruns/payr_adjustments_view/'.$Users->payr_detail_id,'View',array('class'=>'view'), $upd) . ' &nbsp&nbsp| ',
			anchor_popup('payroll/show_payslip/'.$Users->payr_detail_id,'View',array('class'=>'view'), $upd)
	...
	$this->load->library('cezpdf');
		...
		$pdd = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->payr_id;

		
		//$payroll_date = $this->payrollmodel->get_payroll_date($pdd);
		$pdate = $this->payrollmodel->get_payroll_date($pdd);
		$payroll_coverage = $this->payrollmodel->get_payroll_coverage($pdd);
		...
		$employee_no = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->employee_no;
		$last_name = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->last_name;
		$first_name = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->first_name;
		$middle_name = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->middle_name;
		
		$tin = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->tin;
		$sss = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->sss;
		$philhealth = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->philhealth;
		$hdmf = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->hdmf;
		


		$position = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->position;
		$department = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->department;

		$basic_salary = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->basic_salary;
		$salary_adjust = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->salary_adjust;
		$de_minimis_total = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->de_minimis_total;

		$days_absent = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->days_absent;
		$minutes_late = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->minutes_late;


		$tax = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->tax;
		$sss_contri = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->sss_contri;
		$philhealth_contri = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->philhealth_contri;
		$pag_ibig_contri = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->pag_ibig_contri;
		$sss_loan = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->sss_loan;
		$hdmf_loan = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->hdmf_loan;

		$other_deduc = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->other_deduc;
		$total_deduc = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->total_deduc;
		$net_pay = $this->payrollmodel->get_payroll_detail($payr_detail_id)->row()->net_pay;
	...
	{
	 $data['Users'] = (array)$this->payrollmodel->get_by_payr_detail_id($payr_detail_id)->row();
	 $id = $data['Users']['payr_detail_id'];
	 
		
	 // delete user
	 	$this->payrollmodel->delete_payr_detail($payr_detail_id);
	...
	function delete($id)
	{
	 $data['Users'] = (array)$this->payrollmodel->get_by_id($id)->row();
